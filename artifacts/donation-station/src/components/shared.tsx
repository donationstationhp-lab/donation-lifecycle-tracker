import { Cloud, Snowflake, Thermometer, Box, FileCheck, Warehouse, Send } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { DonationItemTier, DonationItemStage, DonationItemCondition, DonationItemTemperatureZone } from '@workspace/api-client-react';
...
export function NumerologyReading({ reading }: { reading?: string | null }) {
  ...
  return (
    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-indigo-950 text-indigo-100 border border-indigo-800 shadow-inner w-fit">
      <svg className="w-3 h-3 text-indigo-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinelinejoin="round">
